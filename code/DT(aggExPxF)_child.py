"""
filename: DT(aggExPxF)_child.py
...
def main():
	...
	#parent_data.to_csv('processed_DT(aggExPxF).csv')
	parent_data = parent_data.sample(frac=1)    # random sampling

	# bootstrapping
	parent_data0 = parent_data[parent_data.prosocial_child != 1]
	parent_data1 = parent_data[parent_data.prosocial_child != 0]
	...
	y0 = np.array(parent_data0['prosocial_child'])
	y1 = np.array(parent_data1['prosocial_child'])
	...
	# with open("DT(aggExPxF)_child.log", "w") as fout:
